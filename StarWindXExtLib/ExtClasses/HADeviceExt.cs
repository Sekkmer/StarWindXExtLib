...
namespace StarWindXExtLib
{
    ...
    internal class HADeviceExt : DeviceExt, IHADeviceExt
    {
        ...
        private void PartnerRefreshed(object sender, EventArgs e)
        {
            ...
            if (lastPartnerCount < ParnerNodesCount) partnersExt.AddRange(Enumerable.Range(lastPartnerCount, ParnerNodesCount).Select(i => new HAParnerExt(this, i) as IHAPartnerExt));
            lastPartnerCount = ParnerNodesCount;
            foreach (var partner in PartnersExt) {
                (partner as HAParnerExt).ParentRefreshed();
            }
        }
        ...
        public ICollection SynchronizationChannels => new CollectionExt<IHANetworkInterface>(GetChannels(NetworkInterfaceType.Synchronization));

        public ICollection HeartbeatChannels => new CollectionExt<IHANetworkInterface>(GetChannels(NetworkInterfaceType.Heartbeat));
        ...
        {
            return SelectChannels(node, NetworkInterfaceType.Synchronization, GetPartnerParameter("sync_channels", node));
        }

        public IEnumerable<IHANetworkInterface> GetPartnerHeartbeatChannels(int node)
        {
            return SelectChannels(node, NetworkInterfaceType.Heartbeat, GetPartnerParameter("heartbeat_channels", node));
        }
        ...
        public IEnumerable<IHANetworkInterface> AllChannels {
            get => GetChannels(NetworkInterfaceType.Synchronization).Concat(GetChannels(NetworkInterfaceType.Heartbeat));
        }
        ...
        public IEnumerable<IHAPartnerExt> PartnersExt => partnersExt ?? (partnersExt = partnersExt = Enumerable.Range(1, ParnerNodesCount).Select(i => new HAParnerExt(this, i) as IHAPartnerExt).ToList());
        ...
        private IEnumerable<IHANetworkInterface> GetChannels(NetworkInterfaceType type)
        {
            ...
            return Enumerable.Range(1, ParnerNodesCount).SelectMany(parnterId =>
                SelectChannels(parnterId, type,
                    GetPropertyValue("ha_partner_node" + parnterId.ToString() + "_" + name + "_channels")));
        }

        private static IEnumerable<IHANetworkInterface> SelectChannels(int parnterId, NetworkInterfaceType type, string channels)
        {
